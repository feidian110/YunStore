<?php
namespace addons\YunStore\merchant\forms;

use addons\YunStore\common\enums\PointExchangeTypeEnum;
use addons\YunStore\common\models\product\AttributeValue;
use addons\YunStore\common\models\product\Product;
use addons\YunStore\common\models\product\Sku;
...
class ProductForm extends Product
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        if ($this->is_attribute == StatusEnum::ENABLED) {
            ...
            // 更新sku
            Yii::$app->yunStoreService->productSku->saveByProductId($this->id, $this->attributes);
        }

        // 更新总库存
        $stock = Yii::$app->yunStoreService->productSku->getStockByProductId($this->id);
        // 更新规格、规格值
        $specModel = Yii::$app->yunStoreService->productSpec->getListWithValue($this->id);
        // 更新记录最小sku
    ...
    protected function updateAttributeValue()
    {
        ...
        // 系统模型的参数
        $baseAttributeValue = Yii::$app->yunStoreService->baseAttributeValue->findByIds($ids);
    ...
    protected function updateSpec($specData)
    {
        ...
        $updatedIds = [];
        // 系统规格
        $baseSpec = Yii::$app->yunStoreService->baseSpec->findByIds($specIds);
        $tmpBaseSpec = [];
    ...
    {
        $sysOptions = Yii::$app->yunStoreService->baseSpecValue->findByIds($valueIds);
    ...
    protected function updateSku()
    {
        ...
        // 查询所有的规格
        $specValue = Yii::$app->yunStoreService->productSpecValue->getListByProductId($this->id);
